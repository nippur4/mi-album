...
export function useMyOffers() {
  ...
  const refetch = useCallback(async () => {
    ...
    setReceived(enriched.filter((o) => o.to_user === uid));
    setSent(enriched.filter((o) => o.from_user === uid));
    setIsLoading(false);
  }, [uid]);

  useEffect(() => { refetch(); }, [refetch]);

  return { received, sent, isLoading, refetch };
}

export function useAlbumMatches(albumId: string | undefined) {
  const [matches, setMatches] = useState<AlbumMatch[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  ...
  const refetch = useCallback(async () => {
    ...
    setMatches(((data ?? []) as any[]) as AlbumMatch[]);
    setIsLoading(false);
  }, [albumId]);
