-- Mi Álbum de Figuritas — canjeo de QR de sobres
--
-- Esta RPC NO valida la firma del QR; eso lo hace la Edge Function `redeem_qr`
-- (ver supabase/functions/redeem_qr/index.ts) que verifica el HMAC con qr_secret
-- y luego invoca esta función con los datos parseados del token.
--
-- Decisión: el QR es fijo (mismo token siempre). El anti-abuso es 100% rate limit
-- por (user, álbum), no expiración del token. Si el owner quiere invalidarlo,
-- usa fn_rotate_qr_secret.
--
-- Comportamiento: si el caller no era miembro del álbum, esta función lo une
-- (con welcome pack si aplica) ANTES de otorgar el sobre del QR — el QR es
-- también un atajo para joinear.
...
-- Devuelve:
--   {
--     packs: int,                 -- sobres QR otorgados
--     next_available_at: ts,      -- próximo escaneo permitido
--     joined: bool,               -- true si se acaba de unir al álbum
--     welcome_packs: int          -- welcome otorgados (0 si ya era miembro)
--   }
--
-- Errores:
...
--   P0097 qr_on_cooldown_until_<ts>
...
begin
  ...
  if v_member.last_qr_redeem_at is not null then
    ...
    if v_next_available > now() then
      raise exception 'qr_on_cooldown_until_%', v_next_available using errcode = 'P0097';
    end if;
  end if;
